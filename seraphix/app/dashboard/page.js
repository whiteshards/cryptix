            ...
            {activeTab === 'statistics' && (
              ...
              >
                ...
                <motion.div
                  ...
                  transition={{ duration: 0.6, delay: 0.1 }}
                >
                  <h2 className="text-white text-3xl font-bold mb-2 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] bg-clip-text text-transparent">
                    Statistics Dashboard
                  </h2>
                  <p className="text-gray-400">Real-time analytics for your keysystems</p>
                </motion.div>
                ...
                {keysystems.length === 0 ? (
                  ...
                  >
                    <motion.div 
                      className="w-20 h-20 bg-gradient-to-br from-[#6366f1]/20 to-[#8b5cf6]/20 rounded-full flex items-center justify-center mx-auto mb-6"
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      transition={{ delay: 0.4, type: "spring" }}
                    >
                    ...
                    <p className="text-gray-400 text-base mb-6">
                      Create some keysystems to start seeing statistics
                    </p>
                    <motion.button
                      onClick={() => setShowModal(true)}
                      className="bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] hover:from-[#5856eb] hover:to-[#7c3aed] text-white px-6 py-3 rounded-lg font-medium transition-all duration-200 shadow-lg"
                      whileHover={{ scale: 1.05, y: -2 }}
                      whileTap={{ scale: 0.95 }}
                    >
                      Create First Keysystem
                    </motion.button>
                  </motion.div>
                ) : (
                  <>
                    ...
                    >
                      {[
                        { 
                          title: 'Total Keysystems', 
                          value: keysystems.length, 
                        ...
                        { 
                          ...
                          value: keysystems.filter(ks => ks.active).length, 
                        ...
                        { 
                          ...
                          value: keysystems.reduce((acc, ks) => acc + (ks.checkpoints?.length || 0), 0), 
                          icon: 'M9 5l7 7-7 7',
                        ...
                        { 
                          title: 'Avg Timer (hrs)', 
                          value: keysystems.length > 0 ? Math.round(keysystems.reduce((acc, ks) => acc + (ks.keyTimer || 0), 0) / keysystems.length) : 0, 
                        ...
                        <motion.div
                          key={index}
                          className={`relative overflow-hidden rounded-2xl border ${stat.bgColor} p-6 backdrop-blur-sm`}
                    ...
                    <motion.div
                      ...
                      transition={{ duration: 0.6, delay: 0.4 }}
                    >
                      {/* Keysystem Status Distribution */}
                      <motion.div 
                        className="relative rounded-2xl border border-white/10 p-6 bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-md"
                        whileHover={{ scale: 1.01 }}
                        transition={{ duration: 0.2 }}
                      >
                        <h3 className="text-white text-lg font-semibold mb-6 flex items-center">
                          <div className="w-2 h-2 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] rounded-full mr-3"></div>
                          Keysystem Status Distribution
                        </h3>
                        <div className="relative h-64 flex items-center justify-center">
                          <div className="relative">
                            ...
                            >
                              {(() => {
                                const activeCount = keysystems.filter(ks => ks.active).length;
                                const inactiveCount = keysystems.length - activeCount;
                                const total = keysystems.length;
                                ...
                                return (
                                  <>
                                    ...
                                    <motion.circle
                                      ...
                                      animate={{ strokeDasharray: `${activeStroke} ${circumference}` }}
                                      transition={{ duration: 1.5, delay: 0.7, ease: "easeOut" }}
                                    />
                                    {/* Inactive systems arc */}
                                    <motion.circle
                                      cx="100"
                                      cy="100"
                                      r={radius}
                                      ...
                                      strokeLinecap="round"
                                      initial={{ strokeDasharray: `0 ${circumference}` }}
                                      animate={{ strokeDasharray: `${inactiveStroke} ${circumference}` }}
                        ...
                        <div className="flex justify-center space-x-6 mt-4">
                          <div className="flex items-center">
                            ...
                            <span className="text-gray-300 text-sm">Active ({keysystems.filter(ks => ks.active).length})</span>
                          </div>
                          <div className="flex items-center">
                            <div className="w-3 h-3 bg-gradient-to-r from-red-500 to-red-600 rounded-full mr-2"></div>
                            <span className="text-gray-300 text-sm">Inactive ({keysystems.filter(ks => !ks.active).length})</span>
                          </div>
                        </div>
                      </motion.div>

                      {/* Timer Distribution Chart */}
                      <motion.div 
                        className="relative rounded-2xl border border-white/10 p-6 bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-md"
                        whileHover={{ scale: 1.01 }}
                        transition={{ duration: 0.2 }}
                      >
                        <h3 className="text-white text-lg font-semibold mb-6 flex items-center">
                          ...
                          Timer Distribution (Hours)
                        </h3>
                        <div className="h-64">
                          <div className="grid grid-cols-1 gap-3 h-full">
                            {(() => {
                              const timerGroups = keysystems.reduce((acc, ks) => {
                                const timer = ks.keyTimer || 0;
                                const group = timer <= 6 ? '0-6h' : timer <= 12 ? '7-12h' : timer <= 24 ? '13-24h' : '24h+';
                                acc[group] = (acc[group] || 0) + 1;
                                return acc;
                              }, {});

                              const maxCount = Math.max(...Object.values(timerGroups), 1);
                              const colors = ['from-blue-500 to-cyan-500', 'from-green-500 to-emerald-500', 'from-yellow-500 to-orange-500', 'from-red-500 to-pink-500'];

                              return Object.entries(timerGroups).map(([group, count], index) => (
                                <div key={group} className="flex items-center space-x-3">
                                  <div className="w-16 text-gray-300 text-sm font-medium">{group}</div>
                                  ...
                                  <motion.div 
                                    className="w-8 text-white text-sm font-bold text-right"
                    ...
                    >
                      <h3 className="text-white text-xl font-semibold mb-6 flex items-center">
                        <div className="w-2 h-2 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] rounded-full mr-3"></div>
                        Individual Keysystem Metrics
                      </h3>
                      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                        {keysystems.map((keysystem, index) => (
                          <motion.div
                            key={keysystem.id}
                            className="relative rounded-2xl border border-white/10 p-6 bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-md hover:border-white/20 transition-all duration-300"
                            initial={{ opacity: 0, y: 20, scale: 0.9 }}
                            animate={{ opacity: 1, y: 0, scale: 1 }}
                          ...
                          >
                            <div className="flex items-start justify-between mb-4">
                              <div className="flex-1 min-w-0">
                                <h4 className="text-white font-semibold truncate">{keysystem.name}</h4>
                                <p className="text-gray-400 text-xs font-mono mt-1">
                                  {keysystem.id.substring(0, 12)}...
                                </p>
                              </div>
                              <motion.span 
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              ...
                              <div className="text-center p-3 rounded-lg bg-white/5 border border-white/10">
                                ...
                                >
                                  {keysystem.keyTimer}h
                                </motion.div>
                              </div>
                              <div className="col-span-2 text-center p-3 rounded-lg bg-white/5 border border-white/10">
                                ...
                                >
                                  {keysystem.checkpoints?.length || 0}
                                </motion.div>
                              </div>
                            </div>

                            {/* Animated background effect */}
                            <div className="absolute inset-0 bg-gradient-to-br from-[#6366f1]/5 to-[#8b5cf6]/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                          </motion.div>
                        ))}
                      </div>
                    </motion.div>
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between lg:space-x-4 space-y-3 lg:space-y-0">
                      <div className="flex-1">
                        ...
                        >
                          ...
                          {keysystems.map((ks) => (
                            <option key={ks.id} value={ks.id} className="bg-[#2a2d47] text-white">
                              {ks.name} ({ks.id})
                            </option>
                          ))}
                        </motion.select>
                        ...
                        <div className="flex items-center justify-between bg-black/20 rounded-lg p-2 border border-white/10">
                          ...
                          <div className="text-right">
                            ...
                            <p className={`font-medium ${
                              ...
                              {Math.round((keysPagination.totalKeys / (keysystems.find(ks => ks.id === selectedKeysystemForKeys)?.maxKeyLimit || 5000)) * 100)}%
                            </p>
                          </div>
                        </div>
                      </motion.div>
          ...
          >
            ...
            >
              <div className="p-4 sm:p-6">
                ...
                <motion.div 
                  className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3 mt-6"
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
