...
export async function GET(request) {
  try {
    ...
    // Find the keysystem that contains this checkpoint token
    const user = await collection.findOne({
      'keysystems.checkpoints.callback_token': token
    });

    if (!user) {
      return NextResponse.json({ error: 'Checkpoint token not found' }, { status: 404 });
    }
    ...
    return NextResponse.json({
      success: true,
      keysystem: {
        id: foundKeysystem.id,
        name: foundKeysystem.name,
        active: foundKeysystem.active,
        checkpointCount: foundKeysystem.checkpoints.length
      },
    ...
    console.error('Identify checkpoint error:', error);
