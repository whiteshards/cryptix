...
export default function ScriptPage({ params }) {
  ...
  // If user has access, show the keysystem interface
  return (
    <div className="min-h-screen bg-[#0f1015]">
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          <div className="bg-[#1a1b2e] rounded-lg border border-white/10 p-6">
            <h1 className="text-2xl font-bold text-white mb-4">{keysystem.name}</h1>
            <p className="text-gray-400 mb-6">Keysystem ID: {id}</p>

            {/* Add your keysystem interface components here */}
            <div className="space-y-4">
              <div className="bg-[#0f1015] rounded-lg p-4 border border-white/5">
                <h3 className="text-white font-semibold mb-2">Keysystem Status</h3>
                <p className="text-gray-400">
                  Status: <span className={`${keysystem.active ? 'text-green-400' : 'text-red-400'}`}>
              ...
              <div className="bg-[#0f1015] rounded-lg p-4 border border-white/5">
                ...
                <p className="text-gray-400">Max Keys Per Person: {keysystem.maxKeyPerPerson}</p>
                <p className="text-gray-400">Key Cooldown: {keysystem.keyCooldown} minutes</p>
                {!keysystem.permanentKeys && (
                  <p className="text-gray-400">Key Timer: {keysystem.keyTimer} hours</p>
                )}
              </div>
            </div>
          </div>
